...
export default function SwapPage() {
  ...
  const handleBankLinked = async (bank: any) => {
    ...
    if (authenticated) {
      await createNewOrder(bank, true)
      setUserProgress('ready')
    } else {
      setUserProgress('bank_linked')
    }
  ...
  const createNewOrder = async (bankData: any, forceCreate = false) => {
    try {
      ...
      // If forceCreate is true, clear any existing order data first
      if (forceCreate) {
        console.log('🔥 Forcing creation of new order, clearing existing order data');
  ...
  const handleConnectWallet = async () => {
    ...
    if (bankDetails) {
      await createNewOrder(bankDetails, true)
      setUserProgress('ready')
    }
  }
      ...
      <main className="flex-1 relative flex flex-col items-center justify-center py-6 px-4 sm:px-8">
        {userProgress === 'initial' ? (
          ...
          <div className="w-full max-w-md relative z-10">
            ...
            <SwapWidgetWrapper 
              onSwapSuccess={(bankDetails) => {
                console.log('🔄 Primary onSwapSuccess handler triggered');
                // Mark that the primary handler was executed successfully
                window.localStorage.setItem('primarySwapHandlerExecuted', Date.now().toString());
                return createNewOrder(bankDetails, true);
              }} 
              onAnalyticEvent={(eventName, data) => {
                // Log all analytic events
                console.log('📊 Analytic Event:', eventName, data);
                
                // Only use as fallback if SWAP_SUCCESS and primary handler didn't execute
                if (eventName === 'SWAP_SUCCESS') {
                  // Check if primary handler was executed in the last 5 seconds
                  const primaryHandlerTimestamp = window.localStorage.getItem('primarySwapHandlerExecuted');
                  const now = Date.now();
                  const fiveSecondsAgo = now - 5000;
                  
                  if (!primaryHandlerTimestamp || parseInt(primaryHandlerTimestamp) < fiveSecondsAgo) {
                    console.log('⚠️ Primary onSwapSuccess handler didn\'t execute recently, using fallback');
                    // Get bank details
                    const storedBank = localStorage.getItem('linkedBankAccount');
                    if (storedBank) {
                      // Create new order with force=true to ensure we always get a fresh order
                      createNewOrder(JSON.parse(storedBank), true)
                        .then(() => console.log('✅ Fallback order creation successful'))
                        .catch(err => console.error('❌ Fallback order creation failed:', err));
                    }
                  } else {
                    console.log('✅ Primary handler executed recently, skipping fallback');
                  }
                  
                  // Clear the flag after 10 seconds to avoid staleness
                  setTimeout(() => {
                    window.localStorage.removeItem('primarySwapHandlerExecuted');
                  }, 10000);
                }
              }}
            />
